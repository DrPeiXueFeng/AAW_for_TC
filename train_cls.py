...
def parse_args():# 解析命令行参数
    ...
    parser.add_argument('--learning_rate', default=0.009, type=float, help='learning rate in training [default: 0.001]')
    parser.add_argument('--gpu', type=str, default='0', help='specify gpu device [default: 0]')
    parser.add_argument('--num_point', type=int, default=1024, help='Point Number [default: 1024]')
    parser.add_argument('--optimizer', type=str, default='Adam', help='optimizer for training [default: Adam]')
    parser.add_argument('--log_dir', type=str, default=None, help='experiment root')
    parser.add_argument('--decay_rate', type=float, default=1e-4, help='decay rate [default: 1e-4]')
    parser.add_argument('--normal', action='store_true', default=False, help='Whether to use normal information [defaulSt: False]')
    # 采用parser对象的parse_args函数解析的参数
...
def main(args):
    ...
    for epoch in range(start_epoch,args.epoch):
        ...
        # optimizer.step()通常在每个mini-batch之中，而scheduler.step()通常在epoch里面，
        # 但也不是绝对的，可以根据具体的需求来做
        # 只有用了optimizer.step(),模型才会更新，而scheduler.step()是对lr进行调整。
        scheduler.step()
        for batch_id, data in tqdm(enumerate(trainDataLoader, 0), total=len(trainDataLoader), smoothing=0.9):
            points, target = data# data:(B, 1024, 6)使用法向量信息就是6，否则就是3
